jQuery( function ( $ )
{
	...
	$( 'body' ).on( 'click', '.mashsb-rwmb-thickbox-upload', function ()
	{
		var $this = $( this ),
			$holder = $this.siblings( '.mashsb-rwmb-images' ),
			post_id = $( '#post_ID' ).val(),
		...
		window.send_to_editor = function ( html )
		{
			...
			html = '<li id="item_' + id + '">';
			html += '<img src="' + url + '">';
			html += '<div class="mashsb-rwmb-image-bar">';
			html += '<a class="mashsb-rwmb-delete-file" href="#" data-attachment_id="' + id + '">×</a>';
			html += '</div>';
